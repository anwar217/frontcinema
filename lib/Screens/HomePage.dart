...
class _HomePageState extends State<HomePage> {
  ...
  void get_categories() async {
    ...
    if (baseUrl != null) {
      var res = await get(Uri.parse("$baseUrl/categorie/"));
      setState(() {
        categories = jsonDecode(res.body);
      });
    } else {
      // Handle the case when BASE_URL is not set
    }
  }
  ...
  void get_movies() async {
    ...
    var baseUrl = dotenv.env['BASE_URL'];
    if (baseUrl != null) {
      var res = await get(Uri.parse("$baseUrl/Film/"));
      setState(() {
 ...
 get_categoriesById(String id) async {
  await dotenv.load(fileName: ".env");
  var baseUrl = dotenv.env['BASE_URL'];
  if (baseUrl != null) {
    var res = await get(Uri.parse("$baseUrl/$id/"));
                      ...
                      Container(
                        ...
                        child: ElevatedButton(
                          ...
                          style: ElevatedButton.styleFrom(
                            backgroundColor: (selectedCategorie == "All")
                                ? Colors.orange
                                : Colors.grey,
                                        ...
                                        children: [
                                          ...
                                          Container(
                                            margin: const EdgeInsets.only(
                                                bottom: 10.0),
                                            child: Text(
                                                 get_categoriesById(movie["categorie"])as String,
                                              style: const TextStyle(
                                          ...
                                          Container(
                                            margin: const EdgeInsets.only(
                                                bottom: 10.0),
                                            child: Text(
                                              parseDate(movie['listProjection']
                                                  [0]["date"]),
                                              style: const TextStyle(
                                                  color: Colors.white),
                                            ),
